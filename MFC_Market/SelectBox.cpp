...
void SelectBox::OnInitialUpdate()
{
	...
	controlTree->InsertItem(TEXT("RECEIPT_REFUNDS"), 5, 5, NULL);
...
void SelectBox::OnTvnSelchanged(NMHDR* pNMHDR, LRESULT* pResult)
{
	...
	*pResult = 0;
	...
	else if (title == TEXT("RECEIPT_REFUNDS")) {
		::PostMessage(AfxGetMainWnd()->GetSafeHwnd(), EDIT_RECEIPT_REFUNDS, EDIT_RECEIPT_REFUNDS, 0);
	}
}
