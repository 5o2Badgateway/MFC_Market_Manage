...
void Receipt_Refund::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_LIST2, Ticket);
}
...
void Receipt_Refund::OnInitialUpdate()
{
	...
	{
		//设置表头
		Ticket.InsertColumn(i, title[i], LVCFMT_LEFT, 150);
	}
}
...
void Receipt_Refund::OnBnClickedButton1()
{
	...
	while (!file.eof())
	{
		...
		while (line.compare(0, 4, "Total", 0, 4))
		{
			char* cstr = new char[line.length() + 1];
			std::strcpy(cstr, line.c_str());
			char* p = std::strtok(cstr, "\t");
			if (i != 0) {
				Ticket.InsertItem(i, (CString)p);
				int j = 1;
				double one_price = 0;
				int num = 0;
				while (p != 0)
				{
					p = std::strtok(NULL, "\t");
					Ticket.SetItemText(i - 1, j, (CString)p);
					if (j == 1)
						num = atoi(p);
					else if (j == 2)
